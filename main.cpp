...
void randomSet(Set* s) {
    ...
    showOutput("The set constructed with the default constructor: ", &t);
    assignSet(s, &t);
    destroySet(&t);
}
...
void specialCaseTests(void) {
    ...
    for (i = 0; i < 10; i += 1) {
        removeSet(&s, i);// if the removed is the first one, there will be some problem
        removeSet(&s, i);
        assert(s.len == 9 - i);
    }
    assert(isEmptySet(&s));
...
void createRandomSetN(int n, Set * a, Set * b) {
    ...
    while (a->len < n) {
        ...
        x = 2 * (5 * n - (rand() % (10 * n)));
        if (isMemberSet(a, x)) { continue; } // try another number  //problem is here!!!!!
